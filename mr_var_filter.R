...
get_mr <- function(exp_id, out_id, vgwas, q){
    ...
    exposure_dat <- merge(
        exposure_dat,
        vgwas %>% dplyr::select(rsid, phi_x1, phi_x2),
        by.x="SNP",
        by.y="rsid"
    )
    # absolute variance effect size var(Y|G=g)
    exposure_dat$phi_x1 <- abs(exposure_dat$phi_x1)
    exposure_dat$phi_x2 <- abs(exposure_dat$phi_x2)

    # define phi threshold using quantiles
    phi_var <- quantile(exposure_dat$phi_x2, q)

    # Drop top q SNPs with IV-exp variance effect
    p_keep <- exposure_dat %>% dplyr::filter(phi_x2 < !!phi_var) %>% dplyr::pull(pval.exposure)
    p_drop <- exposure_dat %>% dplyr::filter(phi_x2 >= !!phi_var) %>% dplyr::pull(pval.exposure)
    p_all <- exposure_dat$pval.exposure
    exposure_dat <- exposure_dat %>% dplyr::filter(phi_x2 >= !!phi_var)
